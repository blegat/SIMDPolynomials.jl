...
@testset "Fuzz MPoly" begin
    for monos in ([PackedMonomial{4,8}(i) for i in 0:2],) #, [Monomial([i]) for i in 0:1])
        x, y, z = monos
        for _ in 1:10000
            pterms = [rand(-2:2)*prod(rand([x, y, z])^i for i in 0:3) for j in 0:5]
            p = sum(pterms)
            ...
            try
                ...
                if !(iszero(p) && iszero(q)) && gcd(pdg, qdg) != one(x)
                    @show typeof(monos)
                    @show p
                    @show q
                    @show g
